...
function ProviderTopBar() {
    ...
    return (
        <>
            {provider && <div className='lg:mx-12 border shadow mb-2'>
                <ImageViewer img={provider.providerLogo} open={openBackground} setOpen={setOpenBackground} />
                <div className='md:flex gap-4'>
                    <div className='w-full lg:h-100 h-60 shadow relative border-b cursor-pointer' style={{width:'600px', height:'auto'}} onClick={() => handleOpen()}>
                        {loading && <Skeleton />}
                    ...
                    <div className='w-full px-4 flex flex-col gap-1 py-4'>
                        ...
                        <p className='flex gap-1 items-center'><span className='font-semibold'>Contact-Number : </span> <span>+94 {provider.phoneNumber}</span></p>
                    </div>
                </div>
            </div>}
        </>
