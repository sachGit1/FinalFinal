...
export const loginProvider = (provider) => async (dispatch) => {
  try {
    ...
    const providerData = await axios({
      method: "POST",
      url: "https://localhost:4000/api/v1/provider/login",
      data: provider,
      config,
    });
...
export const providerRegister = (provider) => async (dispatch) => {
  try {
    ...
    const providerData = await axios({
      method: "POST",
      url: "https://localhost:4000/api/v1/provider/register",
      data: provider,
    });
    axios.defaults.headers.common[
      "Authorization"
...
export const getAllProviders = () => async (dispatch) => {
  try {
    ...
    const providerData = await axios({
      method: "GET",
      url: "https://localhost:4000/api/v1/provider",
    });
...
export const getProviderById = (id) => async (dispatch) => {
  try {
    ...
    const provider = await axios({
      method: "GET",
      url: `https://localhost:4000/api/v1/provider/${id}`,
    });
